...
def setup_experiment():
    ...
    parser.add_argument("-c", "--config", type=str) # a folder with the respective config.jsons
    ...
    task_config = import_module('benchmarl.conf.task', args.task)
    ...
    *conf, _ = parser.parse_args_into_dataclasses(return_remaining_strings=True)
    ...
    if config_file is not None:  # overwrite the default alg and task params according to the config file
        og_algorithm_config, og_task_config = original_conf = parser.parse_json_file(config_file, allow_extra_keys=True)
        if DefaultParams.algorithm == config['_algorithm']:  # only overwrite default alg params with config if user didn't specify new algorithm
            algorithm_config = update_dataclass_defaults(algorithm_config, og_algorithm_config)            
        if DefaultParams.task == config['_task']:  # only overwrite default task params with config if user didn't specify new task
...
def main():
    ...
    task_key = '/'.join(DefaultParams.task.replace('.', '/').split('/')[:2])
    experiment = Experiment(
        task=task_config_registry[task_key].update_config(dataclasses.asdict(task_config)),
        ...
        critic_model_config=import_module('benchmarl.conf.model', DefaultParams.critic_model)(),
