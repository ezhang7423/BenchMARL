...
def setup_experiment():
    ...
    """SETUP CONFIG"""
    parser = HfArgumentParser((Config, MappoConfig))
...
@dataclass
class Config:
    experiment: str = 'base_experiment'
    algorithm: str 
    task: str 
    model: str = 'layers/mlp'
    # model@critic_mode: str = 'layers/mlp'
    seed: int = 42
    wandb: bool = False


def main():
    conf, alg_conf = setup_experiment()
    ...
    from eztils.torch import (
        seed_everything,
    )  # install torch first to uncomment this line (by getting `poetry add eztils[torch]`` as a dependency)
